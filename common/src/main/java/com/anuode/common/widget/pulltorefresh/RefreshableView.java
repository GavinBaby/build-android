package com.anuode.common.widget.pulltorefresh;
...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	public RefreshableView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mHeader = LayoutInflater.from(context).inflate(R.layout.app__widget__pulltorfresh_refreshavleview, null, true);
 ...
 *            监听器的实现。
	...
	public void finishRefreshing() {
		...
		Preferences.setPullToRefreshLastFreshTime(System.currentTimeMillis());
		new HideHeaderTask().execute();
	}
	...
	private void refreshUpdatedAtValue() {
	  ...
	  上次更新时间的毫秒值
	 */
		long lastUpdateTime = Preferences.getPullToRefreshLastFreshTime();
		...
		if (lastUpdateTime == -1) {
			...
			String value = timeIntoFormat + getResources().getString(R.string.sky);
			updateAtValue = String.format(getResources().getString(R.string.updated_at), value);
